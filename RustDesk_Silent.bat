...
for /f "delims=" %%v in ('powershell -NoProfile -Command "try { [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor 3072 } catch { }; $req = [System.Net.WebRequest]::Create('https://github.com/rustdesk/rustdesk/releases/latest'); $req.AllowAutoRedirect = $true; try { $res = $req.GetResponse(); $res.ResponseUri.OriginalString.Split('/')[-1].Trim('v') } catch { 'ERROR' }"') do set version=%%v
...
powershell -NoProfile -Command "try { [Net.ServicePointManager]::SecurityProtocol = [Net.ServicePointManager]::SecurityProtocol -bor 3072 } catch { }; try { (New-Object Net.WebClient).DownloadFile('%download_url%', 'rustdesk_installer.exe') } catch { exit 1 }"
if %errorlevel% neq 0 (
    if "%LANG_TR%"=="1" (
